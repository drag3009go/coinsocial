...
# Импорты из модулей проекта (должны быть корректные пути)
...
async def delete_old_posts_and_messages():
    while True:
        try:
            ...
            cursor.execute("DELETE FROM posts WHERE timestamp < NOW() - INTERVAL '7 days' RETURNING id, media_url")
            ...
            cursor.execute("DELETE FROM messages WHERE timestamp < NOW() - INTERVAL '7 days' RETURNING id, media_urls")
            ...
            # 3. Удаляем файлы-сироты (не привязанные) старше 2 часов
            delete_old_files(2)
            conn.commit()
            conn.close()
            logger.info(f"🧹 Cleanup: deleted {len(old_posts)} old posts and {len(old_messages)} old messages")
...
async def get_conversations(current_user: dict = Depends(get_current_user)):
    ...
    for other_id in other_ids:
        ...
        if user_info and last_msg:
            cursor.execute("SELECT COUNT(*) FROM messages WHERE sender_id = %s AND receiver_id = %s AND is_read = FALSE", (other_id, current_user["id"]))
...
async def search_users(query: str = Query(..., min_length=1), current_user: dict = Depends(get_current_user)):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT id, username, avatar_url, coins FROM users WHERE username ILIKE %s AND id != %s LIMIT 20", (f"%{query}%", current_user["id"]))
    users = cursor.fetchall()
    conn.close()
    return [dict(user) for user in users]
...
async def get_online_users(current_user: dict = Depends(get_current_user)):
    ...
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT id, username, avatar_url FROM users WHERE last_active >= %s AND id != %s ORDER BY username", (five_min_ago, current_user["id"]))
    users = cursor.fetchall()
    conn.close()
    return [dict(user) for user in users]
