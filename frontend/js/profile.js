class ProfileManager {
    ...
    async uploadAvatar(event) {
        ...
        if (!file.type.startsWith('image/')) {
            this.showError('Ïîæàëóéñòà, âûáåðèòå èçîáðàæåíèå');
            return;
        }

        if (file.size > 5 * 1024 * 1024) {
            this.showError('Ðàçìåð ôàéëà íå äîëæåí ïðåâûøàòü 5MB');
            return;
        }
        ...
        try {
            ...
            if (response.ok) {
                ...
                this.showSuccess('Àâàòàð óñïåøíî îáíîâëåí!');
            } else {
    ...
    async updateProfile(event) {
        ...
        if (!username) {
            this.showError('Èìÿ ïîëüçîâàòåëÿ îáÿçàòåëüíî');
            return;
        }

        if (!email) {
            this.showError('Email îáÿçàòåëåí');
            return;
        }
        ...
        if (Object.keys(updateData).length === 0) {
            this.showError('Íåò èçìåíåíèé äëÿ ñîõðàíåíèÿ');
            return;
        }
        ...
        try {
            ...
            if (response.ok) {
                ...
                this.showSuccess('Ïðîôèëü óñïåøíî îáíîâëåí!');
            } else {
