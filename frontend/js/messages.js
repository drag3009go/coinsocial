...
class MessageManager {
    ...
    renderMessages() {
        ...
        container.innerHTML = this.messages.map(msg => {
            ...
            const sendingMark = msg.is_temp ? '<div class="sending">⏳ Отправка...</div>' : '';
            return `
    ...
    async sendMessage() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                // Заменяем временное сообщение на реальное (обновляем id и убираем флаг)
                const tempIndex = this.messages.findIndex(m => m.id === tempId);
                if (tempIndex !== -1) {
                    this.messages[tempIndex].id = data.id;
                    this.messages[tempIndex].is_temp = false;
                    this.renderMessages();
                }
            } else {
