class App {
    constructor() {
        ...
        this.commentTimers = new Map();           // postId -> timer (для автозакрытия)
        this.commentInputTimers = new Map();      // postId -> timer для отслеживания активности
    }
    ...
    async loadFeed(reset = true) {
        ...
        if (reset) {
            ...
            this.commentInputTimers.forEach(timer => clearTimeout(timer));
            this.commentInputTimers.clear();
        }
        this.isLoading = true;
        this.showLoading();
        try {
    ...
    renderFeed() {
        ...
        // Восстанавливаем состояние открытых комментариев и таймеры
        this.posts.forEach(post => {
            ...
            if (commentsList) {
                ...
                if (isOpen) {
                    ...
                    this.startAutoCloseTimer(post.id); // запускаем таймер автозакрытия
                }
            }
            this.attachCommentDraftListenersForPost(post.id);
        });
    ...
    attachCommentDraftListenersForPost(postId) {
        ...
        const saveHandler = (e) => {
            ...
            this.resetAutoCloseTimer(postId); // активность пользователя – сброс таймера
        };
        ...
        // Также слушаем фокус и потерю фокуса для таймеров
        const focusHandler = () => this.resetAutoCloseTimer(postId);
        const blurHandler = () => this.startAutoCloseTimer(postId);
    ...
    resetAutoCloseTimer(postId) {
        ...
        // Если комментарии всё ещё открыты, запускаем новый таймер
        const commentsList = document.getElementById(`comments-list-${postId}`);
        if (commentsList && commentsList.style.display === 'block') {
    ...
    // ---------- Комментарии с деревом + кнопка "Показать ответы" ----------
    ...
    renderCommentTree(comments, postId, level = 0) {
        ...
        return comments.map(comment => {
            ...
            const repliesHtml = hasReplies ? `
                ...
                <button class="show-replies-btn" data-comment-id="${comment.id}" data-post-id="${postId}">
                    ${comment.showReplies ? 'Скрыть ответы' : `Показать ответы (${comment.replies.length})`}
                </button>
            ` : '';
            return `
    ...
    // ---------- Реакции на посты (обновление без перезагрузки ленты) ----------
