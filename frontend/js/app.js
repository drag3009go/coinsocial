class App {
            ...
            if (response.ok) {
                ...
                for (const newPost of newPosts) {
                    ...
                    if (existingPostDiv) {
                        ...
                        // Привязываем обработчик кнопки комментариев
                        const commentBtn = document.querySelector(`.post[data-post-id="${newPost.id}"] .comment-button`);
                        if (commentBtn) {
                            commentBtn.onclick = () => this.toggleComments(newPost.id);
                        }
                    }
                }
            }
    ...
    renderFeed() {
        ...
        // Восстанавливаем состояние открытых комментариев
        ...
        // Привязываем обработчики для кнопок комментариев
        this.posts.forEach(post => {
            const commentBtn = document.querySelector(`.post[data-post-id="${post.id}"] .comment-button`);
            if (commentBtn) {
                commentBtn.onclick = () => this.toggleComments(post.id);
            }
        });
    }
    ...
    attachCommentDraftListenersForPost(postId) {
        ...
        if (input) {
            const handler = (e) => this.saveCommentDraft(postId, e.target.value);
    ...
    async addComment(postId) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                // Открыть комментарии, если они были закрыты
    ...
    async handleLike(postId) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                // обновляем счётчики без перезагрузки всей ленты
                const postDiv = document.querySelector(`.post[data-post-id="${postId}"]`);
                if (postDiv) {
                    const statsSpan = postDiv.querySelector('.post-stats');
                    if (statsSpan) {
                        const likesSpan = statsSpan.querySelector('span:first-child');
                        const dislikesSpan = statsSpan.querySelector('span:nth-child(2)');
                        if (likesSpan && dislikesSpan) {
                            let likes = parseInt(likesSpan.innerText);
                            let dislikes = parseInt(dislikesSpan.innerText);
                            if (result.message === 'Post liked') likes++;
                            else if (result.message === 'Like removed') likes--;
                            else if (result.message === 'Reaction changed to like') { dislikes--; likes++; }
                ...
                if (postDiv) {
                    ...
                    if (statsSpan) {
                        ...
                        if (likesSpan && dislikesSpan) {
                            let likes = parseInt(likesSpan.innerText);
                            let dislikes = parseInt(dislikesSpan.innerText);
                            if (result.message === 'Post disliked') dislikes++;
                            else if (result.message === 'Dislike removed') dislikes--;
                            else if (result.message === 'Reaction changed to dislike') { likes--; dislikes++; }
                            likesSpan.innerText = `${likes} 👍`;
                            dislikesSpan.innerText = `${dislikes} 👎`;
                        }
                    }
    ...
    async createPost(event) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                // добавляем новый пост в начало без перезагрузки всей ленты
