...
class MessageManager {
    ...
    renderConversations() {
        ...
        document.querySelectorAll('.conversation-item').forEach(item => {
            item.addEventListener('click', () => this.selectConversation(item.dataset.peerId));
        });
    }
    ...
    updateChatHeader() {
        ...
        if (this.currentConversation) {
            ...
            if (conv) {
                header.innerHTML = `<div class="user-info"><img src="${conv.avatar_url ? API_BASE + conv.avatar_url : 'default-avatar.png'}" class="avatar"><div class="username">${conv.username}</div></div>`;
            }
    ...
    renderMessages() {
        ...
        container.innerHTML = this.messages.map(msg => {
            ...
            return `<div class="message ${isSent ? 'sent' : 'received'}"><div class="message-content">${this.escapeHtml(msg.content)}</div><div class="message-time">${this.formatTime(msg.timestamp)}</div></div>`;
        }).join('');
    ...
    async sendMessage() {
        ...
        try {
            const response = await fetch(`${API_BASE}/messages/send`, {
                method: 'POST',
                headers: authManager.getAuthHeaders(),
                body: JSON.stringify({ receiver_id: this.currentConversation, content })
            });
            if (response.ok) {
                input.value = '';
    ...
    async searchUsers(query) {
        ...
        try {
            ...
            } else console.error('Search failed', response.status);
    ...
    renderUserList(users) {
        ...
        document.querySelectorAll('#searchResults .conversation-item').forEach(item => {
            item.addEventListener('click', () => this.startNewConversation(item.dataset.peerId));
        });
    }
    ...
    formatTime(timestamp) { return new Date(timestamp).toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit' }); }
    ...
    showError(message) {
        ...
        errorDiv.style.position = 'fixed'; errorDiv.style.top = '20px'; errorDiv.style.right = '20px'; errorDiv.style.zIndex = '1000';
